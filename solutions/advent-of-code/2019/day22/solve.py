...
def solve_part2(operations, length=119315717514047, times=101741582076661):
    position = 2020
    a, b = stack_operations(
        [convert_operation(operation, length) for operation in operations], length
    )
    ...
    while times > 0:
        if times & 1 == 1:
            a_total, b_total = stack_operations([(a, b), (a_total, b_total)], length)
...
if __name__ == "__main__":
    ...
    print(f"Part 1 (naive): {timeit(lambda: solve_part1(operations))}")
    print(
        f"Part 1 (inverse transformations): {timeit(lambda: solve_part1_alt(operations))}"
    )
    print(
        f"Part 1 (stacked inverse transformations): {timeit(lambda: solve_part1_alt_alt(operations))}"
    )
    print(f"Part 2: {timeit(lambda: solve_part2(operations))}")
